...
function usePasswordGrant(done) {
  ...
  return this.model.getUser(uname, pword, function (err, user) {
    if (err) return done(error(err.error || 'server_error',
...
function useExtendedGrant(done) {
  ...
  this.model.extendedGrant(this.grantType, this.req,
    function (err, supported, user, extendedTokenAttributes) {
      if (err) return done(error(err.error || 'server_error',
      err.description || err.message, err));
      ...
      self.user = user;
      self.extendedTokenAttributes = extendedTokenAttributes;
      done();
    });
}
...
function sendResponse(done) {
  ...
  if (typeof this.extendedTokenAttributes === 'object') {
    var attributeKeys = Object.keys(this.extendedTokenAttributes);
    for (var i = 0; i < attributeKeys.length; i++) {
      var attributeKey = attributeKeys[i];
      var attributeValue = this.extendedTokenAttributes[attributeKey];
      response[attributeKey] = attributeValue;
    }
  }
